...
```bash
swift package add-dependency https://github.com/googleapis/swift-google-rpc-context.git --from 0.0.0-preview
```

Then add `GoogleRpcContext` to your target's dependencies:

```bash
swift package add-target-dependency GoogleRpcContext <target-name> --package swift-google-rpc-context
```
